...
Options:
  ...
  -f
    By default, existing repositories are not deleted(FORCE=0).
    By default, the script downloads every repository (download=1), skipping existing repositories.
    The -f option only works when download=1 and is used to delete existing repositories before downloading.

  -r
    Delete the folders generated by each code repository during the toolchain building process, including build, install, etc.
    By default, these folders are not deleted, that is, REMOVE_BUILD_FOLDER=0.
