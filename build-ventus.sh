...
Options:
  --build <build programs>
    ...
    Default : "systemc;llvm;ocl-icd;libclc;spike;rtlsim;cyclesim;driver;pocl;rodinia;test-pocl"

  -d
    'BUILD_TYPE' defaults to 'Release', and is 'Debug' when the option is enabled.

  -f
    'FORCE' defaults to 0. When the option is enabled, 'FORCE' is set to 1 to remove existing repositories.
    The option only works when 'download=1' and is used to remove existing repositories before cloning.
    By default, the script clones every repository ('download=1'), skipping existing repositories.

  -r
    Remove the folders generated by each repository during the toolchain building process, including build, install, etc.
    'REMOVE_BUILD_FOLDER' defaults to 0. When the option is enabled, 'REMOVE_BUILD_FOLDER' is set to 1 to remove these folders.
...
while [ $# -gt 0 ]; do
  ...
  -f)
    # Forcefully remove all existing repositories.
    FORCE=1
    ;;

  -r)
    # Remove the old toolchain and rebuild the new one.
    REMOVE_BUILD_FOLDER=1
    ;;
...
echo -e "\n=== Cloning complete! Time taken:${clone_duration} seconds ==="

# Need to get the systemc folder from environment variables
...
SYSTEMC_BUILD_DIR=${SYSTEMC_DIR}/build

# Need to get the ventus-llvm folder from environment variables
...
# Need to get the cpp-cycle-level-simulator folder from environment variables
...
# Build ventus Chisel RTL sim-verilator
build_gpgpu_rtlsim() {
...
build_icd_loader() {
  cd ${OCL_ICD_DIR}
  ./bootstrap
  mkdir -p ${OCL_ICD_BUILD_DIR}
  cd ${OCL_ICD_BUILD_DIR}
...
test_pocl() {
   ...
   mkdir -p test_vecadd && cd test_vecadd
   ../vecadd
   cd "${POCL_BUILD_DIR}/examples/matadd"
   mkdir -p test_matadd && cd test_matadd
   ../matadd
   VENTUS_BACKEND=cyclesim ../matadd
   VENTUS_BACKEND=rtlsim ../matadd
}

# Export needed path and environment variables
export_elements() {
...
# Remove the old build folders and rebuild the new toolchain
if [ "$REMOVE_BUILD_FOLDER" -ne 0 ]; then
  local build_dirs=(
