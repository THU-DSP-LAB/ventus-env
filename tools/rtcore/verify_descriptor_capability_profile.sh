...
SOURCE="${ROOT_DIR}/tools/rtcore/descriptor_capability_probe.c"
OUT_DIR="${OUT_DIR:-/tmp/ventus-descriptor-capability-profile}"
CC="${CC:-cc}"
...
"${CC}" -std=c11 -Wall -Wextra -Werror \
  "${vulkan_cflags[@]}" \
  "${SOURCE}" \
  -o "${OUT_DIR}/descriptor_capability_probe" \
  "${vulkan_libs[@]}"

probe_output="$(
  env \
    VK_ICD_FILENAMES="${ICD}" \
    "${OUT_DIR}/descriptor_capability_probe"
)"
printf '%s\n' "${probe_output}"
rg -q '^PASS descriptor-capability-profile ' <<<"${probe_output}" ||
  die "descriptor capability probe did not report PASS"
